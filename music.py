...
class MusicPlayer(commands.Cog,name='Music'):
    ...
    async def music_voice(self,user,before,after):
        ...
        if after.channel is None and user.id == self.bot.user.id:
            try:
                ...
                print(f"Не удалось получить айди {user.guild.id}") #Server ID lost or was not in data before disconnecting
    ...
    async def queue(self,msg,song):
        ...
        if data['queue']:
            ...
            return await msg.send(f"Добавляет плейлист {data['title']} в очередь")
    ...
    async def start_song(self,msg,song):
        ...
        emb=discord.Embed(colour=self.random_color, title='Теперь Играем',description=download.title,url=download.url)
        emb.set_thumbnail(url=download.thumbnail)
        emb.set_footer(text=f'По запросу {msg.author.display_name}',icon_url=msg.author.avatar_url)
        loop=asyncio.get_event_loop()
    ...
    async def before_play(self,msg):
        ...
        if msg.author.voice is None:
            return await msg.send('**Пожалуйста, присоединяйтесь к голосовому каналу для воспроизведения музыки**'.title())
        ...
        if msg.voice_client.channel != msg.author.voice.channel:
            ...
            if self.player[msg.guild.id]['queue']:
                ...
                return await msg.send("Пожалуйста, присоединяйтесь к тому же голосовому каналу, что и бот, чтобы добавить песню в очередь")
    ...
    async def repeat(self,msg):
        ...
        return await msg.send("Бот не в голосовом канале и не играет музыку")
    ...
    async def reset(self,msg):
        ...
        """
        if msg.voice_client is None:
            return await msg.send(f"**{msg.author.display_name}, в настоящее время из бота не воспроизводится музыку.**")
        ...
        if self.player[msg.guild.id]['queue'] and msg.voice_client.is_playing() is False:
            return await msg.send("**Нет звука в данный момент воспроизводимой музыки в очереди**".title(),delete_after=25)
    ...
    async def skip(self,msg):
        ...
        if msg.author.voice is None or msg.author.voice.channel != msg.voice_client.channel:
            return await msg.send("Пожалуйста, присоединяйтесь к тому же голосовому каналу, что и бот")
        
        
        if self.player[msg.guild.id]['queue'] and msg.voice_client.is_playing() is False:
            return await msg.send("**Нет песен в очереди, чтобы пропустить**".title(),delete_after=60)
    ...
    async def stop(self,msg):
        ...
        """
        if msg.voice_client is None:
            return await msg.send("Бот не подключается к голосовому каналу")

        if msg.author.voice is None:
            return await msg.send("Вы должны быть в том же голосовом канале, что и бот")
        ...
        if msg.author.voice is not None and msg.voice_client is not None:
            ...
            return await msg.send(f"**{msg.author.display_name}, в данный момент нет ни звука, ни песен в очереди**")
    ...
    async def _queue(self,msg):
        ...
        if msg.voice_client is not None:
            if msg.guild.id in self.player:
                if self.player[msg.guild.id]['queue']:
                    ...
                    emb.set_footer(text=f'Команда, используется {msg.author.name}',icon_url=msg.author.avatar_url)
        ...
        return await msg.send("Никаких песен в очереди")
    ...
    async def join(self, msg, *, channel: discord.VoiceChannel=None):
        ...
        """
        if msg.voice_client is not None:
            return await msg.send(f"Бот уже находится в голосовом канале\nВы хотели использовать {msg.prefix}перейти")
    ...
    async def before_join(self,msg):
        if msg.author.voice is None:
            return await msg.send("Вы не находитесь в голосовом канале")
    ...
    async def join_error(self,msg,error):
        ...
        if error.args[0] == 'Команда вызвала исключение: исключение: воспроизведение':
            return await msg.send("**Пожалуйста, присоединяйтесь к тому же голосовому каналу, что и бот, чтобы добавить песню в очередь**".title())
    ...
    async def volume(self,msg,vol:int):
        ...
        return await msg.send("**Пожалуйста, присоединяйтесь к тому же голосовому каналу, что и бот, чтобы использовать команду**".title(),delete_after=30)
    ...
    async def volume_error(self,msg,error):
        if isinstance(error,commands.MissingPermissions):
            return await msg.send("Управление каналами или административная завивка, необходимая для изменения громкости",delete_after=30)
